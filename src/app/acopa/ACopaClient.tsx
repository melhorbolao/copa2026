...
export function ACopaClient({ initialMatches, isAdmin, initialOfficialTopScorer, standardizedNames, r1Deadline }: Props) {
  ...
  return (
    <div>
      {/* ── Botão admin: recalcular pontuações ──────────────────────────────── */}
      {isAdmin && <RecalcButton />}
...
function RecalcButton() {
  ...
  return (
    <div className="mb-3 flex items-center gap-2">
      ...
      {status === 'ok'    && <span className="text-xs text-verde-600 font-medium">✓ Recalculação iniciada</span>}
      {status === 'error' && <span className="text-xs text-red-500 font-medium">Erro ao iniciar</span>}
    </div>
  )
}
