...
export function JogosDashboard({
  ...
  const groupStandings = useMemo(() => {
    ...
    // Group standings use official scores only (empty bet map — bets not relevant here)
    const betMap = new Map<string, import('@/lib/bracket/engine').BetSlim>()
    return calcGroupStandings(
      groupMatches.map(m => ({ id: m.id, group_name: m.group_name!, phase: m.phase, team_home: m.team_home, team_away: m.team_away, flag_home: m.flag_home, flag_away: m.flag_away, score_home: m.score_home, score_away: m.score_away })),
      betMap,
    )
