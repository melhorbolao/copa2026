...
export function BetStats({ match, matchBets, participants, minorityMap, abbr }: Props) {
  ...
  return (
    <div className="rounded-2xl bg-white shadow-sm border border-gray-100 overflow-hidden">
      ...
      {/* Column header (1 / X / 2) */}
      <div className="grid grid-cols-3 border-b border-gray-100 text-center text-[11px] font-bold text-gray-500 px-4 py-1.5 gap-1">
        {(['H', 'D', 'A'] as const).map(r => (
          <div key={r} className={`flex flex-col items-center ${minorityMap?.[r] ? 'text-gray-900' : ''}`}>
            <div className="flex items-center gap-1">
              <span>{r === 'H' ? abbr(match.team_home) : r === 'A' ? abbr(match.team_away) : 'X'}</span>
              {minorityMap?.[r] && <img src="/zebra.png" alt="zebra" width={12} height={12} className="object-contain" />}
            </div>
            <span className={`text-base font-black ${minorityMap?.[r] ? 'text-gray-900' : 'text-gray-700'}`}>
      ...
      <div className="divide-y divide-gray-50">
        {groups.map(g => {
          const resultCol = g.result === 'H'
            ? 0 : g.result === 'D' ? 1 : 2
          return (
            <div
              key={`${g.score_home}-${g.score_away}`}
              className={`flex items-center px-4 py-1.5 gap-2 text-sm ${g.isExact ? 'bg-emerald-50' : g.isImpossible ? 'bg-rose-50/60' : ''}`}
            >
              {/* Score pill */}
              <span className={`font-mono font-bold tabular-nums text-xs w-10 text-center rounded px-1 ${
                g.isExact ? 'text-emerald-700'
              ...
              <div className="flex-1 relative h-1.5 rounded-full bg-gray-100 overflow-hidden">
                <div
                  className={`absolute left-0 top-0 h-full rounded-full ${g.isExact ? 'bg-emerald-400' : g.isImpossible ? 'bg-rose-300' : 'bg-blue-300'}`}
                  style={{ width: `${g.pct}%` }}
                />
              </div>
