...
export async function buildTabelaMBBuffer(
  ...
  const [
    ...
    admin.from('matches')
      .select('id, match_number, round, phase, group_name, team_home, team_away, betting_deadline')
  ...
  const totalCols = Math.max(2 + participants.length, 3)
  ...
  setHdr(1, 'Chave')
  setHdr(2, 'Descrição')
  participants.forEach((p: any, i) => setHdr(3 + i, p.apelido))
  ...
  for (const g of GROUP_ORDER) {
    addDataRow(`grp_3rd:${g}`, `Grupo ${g} – 3º Lugar`,
  ...
  addDataRow('trn:scorer', 'Artilheiro',
